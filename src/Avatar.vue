...
const props=withDefaults(defineProps<{equipment:Equipment;perform?:number;reduced?:boolean;mini?:boolean;emotion?:Emotion;reaction?:number}>(),{perform:0,reduced:false,mini:false,emotion:'idle',reaction:0});
...
const pose=computed(()=>evaluatePose(clock.value,active.value,age.value,complete.value,minimal.value));
...
function updateLoop(){cancelAnimationFrame(raf);raf=0;if(!minimal.value&&inView&&document.visibilityState==='visible')raf=requestAnimationFrame(frame);}
...
<template>
<div ref="host" class="avatar bone-avatar" :class="{mini, 'bone-reduced':minimal}" :data-emotion="active" :data-rig-version="complete==='iron'?'pending':'3'" :data-skin="complete" role="img" :aria-label="`${complete==='mixed'?'我的混搭冒险家':theme.name}，${expressionNames[active]}`">
 <RasterRig v-if="complete!=='iron'" :equipment="equipment" :pose="pose" :emotion="active" :time="minimal?0:clock"/>
