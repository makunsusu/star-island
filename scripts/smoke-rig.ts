...
try{
 ...
 await page.goto(url);const actor=page.locator('.character-stage .bone-avatar');await expect(actor).toBeVisible();await expect(actor).toHaveAttribute('data-rig-version','3');
 for(const layer of ['head','top','pants'])await expect(actor.locator(`[data-layer="${layer}"]`).first()).toBeAttached();
 ...
 await expect.poll(async()=>actor.locator('[data-layer="head"]').getAttribute('href'),{timeout:7000,intervals:[30]}).toMatch(/happy.webp$/);
 await page.getByRole('button',{name:'♡ 害羞',exact:true}).click();await expect(actor).toHaveAttribute('data-emotion','blush');await expect(actor.locator('[data-layer="head"]')).toHaveAttribute('href',/blush.webp$/);
 ...
 await page.getByRole('button',{name:'回到冒险',exact:true}).click();await expect(actor.locator('[data-bone="ribbon-right"]')).toBeAttached();await page.screenshot({path:'work/rig-nezha.png',fullPage:true});
